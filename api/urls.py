...
TODO #231: Are these docs autogenerated? Service and Addon URLs need to go here.
...
urlpatterns = patterns(
    ...
    # service providers
    url(r'^service_providers/(?P<type>[-_\w]+)/?',
        views.ServiceProviderViewSet.as_view({
            'get': 'retrieve', 'put': 'update', 'delete': 'destroy'})),
    url(r'^service_providers/?',
        views.ServiceProviderViewSet.as_view({'get': 'list'})),
    # application release components
    ...
    url(r'^apps/(?P<id>[-_\w]+)/addons/(?P<type>[-_\w]+)/?',
        views.AppServiceViewSet.as_view({
